...
class aknile extends Table
{
    ...
    */
    ...
    {    
        ...
        self::setGameStateInitialValue( 'floodType', 0 );
        self::setGameStateInitialValue( 'floodCardId', 0 );
        self::setGameStateInitialValue( 'iterations', 0 ); //times deck has been exhausted
        self::setGameStateInitialValue( 'gameOverTrigger', 0 ); //set when the deck is emptied for the last time. Checked at end of relevant states.
        self::setGameStateInitialValue( 'plagueTrigger', 0 ); //set when the a plague card is drawn. Checked before moving state
        ...
        {
            if ($cardType['isPlague'] == 1)
            {
                $plagueType = $cardType["type_id"];
                $num = 1;
            }
            else if ($cardType['isSpeculation'] == 1)
            {
                $num = 1;
            }
            else
    ...
    */
    protected function getAllDatas()
    {
        ...
        $plague = reset($cards);
        $result['isPlagueInDeck'] = $plague["location"] == "deck";
...
////////////    
    ...
    */
    ...
    function isSpeculationCard($card)
    {
        $cardType = $this->card_types[$card['type']];
        return $cardType['isSpeculation'] == 1;
    }
    ...
    function drawCardsWithGameOverCheck($numCards, $source, $destination, $dest_type, $isPlayerDraw)
    {
        ...
        while ($numCards > 0 && $this->cards->countCardInLocation('deck') > 0)
        {
            ...
            {
                if ($this->card_types[$card['type']]['isPlague'] == 1)
                {
...
//////////// 
    ...
    /*
        ...
        (note: each method below must match an input method in aknile.action.php)
    */
    ...
    */
    ...
    function plant($card_ids)
    {
        ...
        */
        ...
        foreach($cards as $card)
        {
            if ($this->isSpeculationCard($card))
            {
    ...
    */
    ...
    function stFlood()
    {
        ...
        foreach($this->card_types[$floodType]["harvestTypes"] as $harvestType)
        {
            ...
            foreach($this->card_types as $cardType)
            {
                if (in_array($harvestType, $cardType["harvestTypes"]) && $cardType["isSpeculation"] == 1)
                {
                    self::debug("match");
